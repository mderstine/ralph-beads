...
echo "=== GitHub Project Board Sync ==="
...
# Maps beads priority (int) to project field option name
PRIORITY_MAP = {
    0: "P0 Critical",
    1: "P1 High",
    2: "P2 Medium",
...
def gql(query, **variables):
    ...
    if result.returncode != 0:
        ...
        # Return None for errors instead of crashing
...
def sync_issues_to_board(project_id, fields):
    """Add beads-linked GitHub issues to the project board and set fields."""
    # Get beads issues with external refs
    beads_issues = []
    ...
    # Filter to issues that have GitHub issue linkage
    linked = []
    ...
    # Get existing project items and GH issue node IDs
    ...
    for gh_num, issue in linked:
        ...
        if gh_num in existing_items:
            # Item already on board — check if fields need updating
...
def main():
    project_id, fields = setup_project()
    ...
    if project_id:
        sync_issues_to_board(project_id, fields)
